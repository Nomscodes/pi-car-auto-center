...
/**
 * Formulário de cadastro de cliente — toggle PF/PJ, grid 2 colunas.
 *
 * @author Cassiano
 */
import javax.swing.*;
...
public class PanelCadastroCliente extends JPanel {
    ...
    private boolean modoEmpresa = false;

    // Campos PF
    private JTextField txtNome, txtCPF, txtRG, txtNasc, txtTelefone, txtEmail, txtEndereco;
    // Campos adicionais PJ
    private JTextField txtRazaoSocial, txtCNPJ, txtInscricaoEst;

    private JPanel    painelCampos;
    private JButton   btnPF, btnPJ;
    ...
    private JPanel criarTopbar() {
        ...
        JLabel lbl = new JLabel("AV CAR AUTO CENTER  —  Cadastro de Cliente");
    ...
    private JScrollPane criarScrollConteudo() {
        ...
        // Toggle PF / PJ
        conteudo.add(criarTogglePFPJ());
        conteudo.add(Box.createVerticalStrut(20));

        // Painel de campos (troca conforme o toggle)
        painelCampos = new JPanel();
        painelCampos.setOpaque(false);
        painelCampos.setLayout(new BoxLayout(painelCampos, BoxLayout.Y_AXIS));
        painelCampos.setAlignmentX(Component.LEFT_ALIGNMENT);
        construirCamposPF();
        conteudo.add(painelCampos);

        // Rodapé
        conteudo.add(Box.createVerticalStrut(20));
        conteudo.add(criarRodapeAcoes());
    ...
    // ── Toggle ────────────────────────────────────────────────────────────────
    private JPanel criarTogglePFPJ() {
        JPanel p = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0));
        p.setOpaque(false);
        p.setAlignmentX(Component.LEFT_ALIGNMENT);

        btnPF = criarBotaoToggle("Pessoa Física", true);
        btnPJ = criarBotaoToggle("Pessoa Jurídica", false);

        btnPF.addActionListener(e -> { modoEmpresa = false; atualizarToggle(); construirCamposPF(); });
        btnPJ.addActionListener(e -> { modoEmpresa = true;  atualizarToggle(); construirCamposPJ(); });
    ...
    private JButton criarBotaoToggle(String texto, boolean ativo) {
        JButton btn = new JButton(texto) {
            @Override protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                boolean isAtivo = (texto.contains("Física") && !modoEmpresa)
                               || (texto.contains("Jurídica") && modoEmpresa);
                g2.setColor(isAtivo ? MainFrame.COR_NAVY : Color.WHITE);
                g2.fillRoundRect(0, 0, getWidth(), getHeight(), 8, 8);
                g2.setColor(MainFrame.COR_BORDER);
                g2.setStroke(new java.awt.BasicStroke(1f));
                g2.drawRoundRect(0, 0, getWidth() - 1, getHeight() - 1, 8, 8);
                g2.setColor(isAtivo ? Color.WHITE : MainFrame.COR_NAVY);
    ...
    // ── Campos PF ─────────────────────────────────────────────────────────────
    private void construirCamposPF() {
        painelCampos.removeAll();

        txtNome     = criarCampo(); txtCPF   = criarCampo();
        txtRG       = criarCampo(); txtNasc  = criarCampo();
        txtTelefone = criarCampo(); txtEmail = criarCampo();
        ...
        painelCampos.add(grid);
        painelCampos.revalidate();
        painelCampos.repaint();
    }

    // ── Campos PJ ─────────────────────────────────────────────────────────────
    private void construirCamposPJ() {
        painelCampos.removeAll();

        txtRazaoSocial = criarCampo(); txtCNPJ         = criarCampo();
        txtInscricaoEst = criarCampo(); txtTelefone    = criarCampo();
        txtEmail        = criarCampo(); txtEndereco    = criarCampo();

        JPanel grid = new JPanel(new GridLayout(4, 2, 14, 10));
        grid.setOpaque(false);
        grid.setAlignmentX(Component.LEFT_ALIGNMENT);

        grid.add(criarGrupo("Razão Social",        txtRazaoSocial));
        grid.add(criarGrupo("CNPJ",                txtCNPJ));
        grid.add(criarGrupo("Inscrição Estadual",  txtInscricaoEst));
        grid.add(criarGrupo("Telefone",            txtTelefone));
        grid.add(criarGrupo("E-mail",              txtEmail));
        grid.add(criarGrupo("Endereço",           txtEndereco));
        grid.add(new JPanel() {{ setOpaque(false); }});
    ...
    private JPanel criarRodapeAcoes() {
        ...
        JButton btnSalvar = criarBotaoGold("Salvar cliente", 130, 36);
